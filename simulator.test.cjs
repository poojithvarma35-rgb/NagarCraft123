...
test('strict validation rejects forged assets, overlap, river sites and invalid city limits',()=> {
  ...
  const tooManyRoads=fixture();for(let i=0;i<101;i++)tooManyRoads.roads.push(road('extra'+i,0,0,1000,650));assert.throws(()=>E.validateCity(tooManyRoads),/Maximum/);
  assert.equal(Object.hasOwn(E.analyze(fixture()),'score'),false);
});
test('construction tools use lakh-scale prices capped at five lakh',()=> {
  const costs=Object.values(E.catalog).map(item=>item.cost);
  assert.ok(Math.min(...costs)>=0.01);
  assert.ok(Math.max(...costs)<=0.05);
  assert.equal(E.catalog.residential.cost,0.02);
  assert.equal(E.catalog.industrial.cost,0.05);
  assert.equal(E.roadCost(road('r',0,0,200,0)),0.01);
});
